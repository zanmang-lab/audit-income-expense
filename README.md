...
학생회 감사 자료용 **수입지출내역서**를 엑셀 장부·연체료/환불 목록·연체료 규정 이미지에서 자동 생성하는 도구입니다.
...
### 연체료 규정 이미지 인식 실패
- 표가 선명한 사진·스캔인지 확인
- `/api/health`에서 `parser_build`가 `table-format-v2`인지 확인
- 서버 중복 실행 시 `run_web.bat`으로 재시작 (포트 8000 정리 포함)
